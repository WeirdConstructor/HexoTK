...
impl HexGridModel for TestGrid {
    ...
    fn cell_visible(&self, _x: usize, _y: usize) -> bool {
        true
    }
    fn cell_empty(&self, _x: usize, _y: usize) -> bool {
        false
    }
    ...
    fn cell_label<'a>(&self, _x: usize, _y: usize, _out: &'a mut [u8]) -> Option<HexCell<'a>> {
        None
    }
    fn cell_edge<'a>(&self, _x: usize, _y: usize, _edge: HexDir) -> HexEdge {
        HexEdge::Arrow
    }
...
fn main() {
    ...
    open_window_ext(
        ...
        Box::new(|| {
            ...
            wtd.set_data_source(
                "XXX",
                Rc::new(vec![0.0, 0.1, 0.11, 0.2, 0.4, 0.1, 0.9, 0.8, 0.4, 0.0]),
            );
            ...
            wtwid.reg("click", |_ctx, _wid, ev| match &ev.data {
            ...
            let lbl1 = Widget::new(style_ref.with_style_clone(|style| {
                style.border = 0.0;
                style.pad_left = 5.0;
            ...
            let knob = Widget::new(style_ref.with_style_clone(|style| {
                style.pad_top = 20.0;
                style.pad_left = 30.0;
            }));
            knob.enable_cache();
            ...
            pedit.set_ctrl(Control::PatternEditor { edit: pateditor });
            ...
            for xi in 0..50 {
                gd.borrow_mut().set_point(
                    xi,
                    (((xi as f64 / 50.0) * std::f64::consts::PI * 2.0).sin() + 1.0) * 0.5,
                );
            }
        ...
        baseview::WindowScalePolicy::ScaleFactor(0.75), // .25)
