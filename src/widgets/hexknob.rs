...
impl Knob {
    ...
    pub fn get_decor_rect1(&self) -> (f32, f32, f32, f32) {
        (
            ...
            (self.sbottom.1 - 0.5 * self.line_w).round(),
            ((self.s[6].0 - self.s[0].0).abs() + 0.6 * self.line_w).round(),
            self.line_w * 1.0,
        )
    }
    ...
    pub fn draw_oct_arc(
        ...
        let (next_idx, prev_arc_len) = if value > arc_len[4] {
            (6, arc_len[4])
...
impl ParamModel for DummyParamModel {
    ...
    fn change(&mut self, v: f32, res: ChangeRes) {
        match res {
            ...
            ChangeRes::Fine => {
                self.value = (v * 100.0).round() / 100.0;
            }
            ChangeRes::Coarse => {
                self.value = (v * 10.0).round() / 10.0;
            }
        }
    }
    fn change_end(&mut self, v: f32, res: ChangeRes) {
    ...
    fn fmt_mod<'a>(&self, buf: &'a mut [u8]) -> usize {
        let modamt = if let Some(ma) = self.modamt {
            ma
        } else {
            return 0;
        };
...
impl HexValueDrag {
    fn from_state(
        ...
        model: &mut dyn ParamModel,
        btn: MButton,
        zone: HexKnobZone,
    ) -> Option<Self> {
        let is_modamt = MButton::Right == btn.into();

        let (res, step_dt) = match zone {
            HexKnobZone::Coarse => (ChangeRes::Coarse, model.get_ui_steps().0),
            HexKnobZone::Fine => (ChangeRes::Fine, model.get_ui_steps().1),
        };
        ...
        Some(Self {
            ...
            mouse_start: (modkeys.mouse.x, modkeys.mouse.y),
        })
    }
...
impl HexKnob {
    ...
    pub fn handle(&mut self, w: &Widget, event: &InputEvent, out_events: &mut Vec<(usize, Event)>) {
        ...
        match event {
            ...
            | InputEvent::MouseButtonPressed(MButton::Right) => {
                ...
                let btn = if let InputEvent::MouseButtonPressed(btn) = event {
                    *btn
                } else {
                    MButton::Left
                };
                ...
                if let Some(zone) = self.cursor_zone(self.modkeys.mouse.x, self.modkeys.mouse.y) {
                    if let Some(mut hvd) =
                        HexValueDrag::from_state(&self.modkeys, &mut *model, btn, zone)
                    {
                        hvd.start(&mut *model);
                        self.drag = Some(hvd);
            ...
            InputEvent::MouseButtonReleased(btn) => {
                ...
                if let Some(mut hvd) = self.drag.take() {
                    hvd.end(&mut *model, self.modkeys.mouse.x, self.modkeys.mouse.y);
            ...
            InputEvent::MousePosition(x, y) => {
                if let Some(ref mut hvd) = self.drag {
                    hvd.change(&mut *model, self.modkeys.mouse.x, self.modkeys.mouse.y);
            ...
            InputEvent::MouseWheel(y) => {
                ...
                if let Some(zone) = self.cursor_zone(self.modkeys.mouse.x, self.modkeys.mouse.y) {
                    if let Some(mut hvd) =
                        HexValueDrag::from_state(&self.modkeys, &mut *model, MButton::Left, zone)
                    {
                        hvd.start(&mut *model);
            ...
            InputEvent::KeyPressed(key) => {
                if Key::Shift == key.key {
                    ...
                    if let Some(ref mut hvd) = self.drag {
                        hvd.set_fine_res(&mut *model, self.modkeys.mouse.x, self.modkeys.mouse.y);
                    }
                }
            }
            _ => {}
    ...
    pub fn draw(&mut self, w: &Widget, _style: &Style, pos: Rect, real_pos: Rect, p: &mut Painter) {
        ...
        let (xo, yo) = ((pos.x + pos.w / 2.0).round(), (pos.y + pos.h / 2.0).round());
        ...
        let zone_hover = if let Some(hvd) = &self.drag {
            Some(hvd.zone)
        } else if is_hovered {
            self.hover
        } else {
        ...
        let highlight = if !model.enabled() {
            HLStyle::Inactive
        ...
        match highlight {
            ...
            HLStyle::Hover(fine) => {
                if fine {
                    let r = self.knob.get_fine_adjustment_mark();
                    p.rect_fill(UI_TXT_KNOB_HOVER_CLR, xo + r.0, yo + r.1, r.2, r.3);
                }
